<div class="bg-gray-700">
    <div class="mx-12 flex flex-col items-center bg-slate-400 h-fit">
        <div class="my-12">
            <div class="text-center">
                <p class="text-4xl">Contacts</p>
                <br>
                <p class="text-2xl">Please send your useless inquiries to our email that we don’t use.</p>
                <br>
                <a class="text-6xl" href="Garbage@ProWebber.ca">Garbage@ProWebber.ca</a>
            </div>
        </div>
        <div class="my-12">
            <div class="text-center">
                <form>
                    <p class="text-3xl footer-title">Email Form</p>
                    <fieldset class="w-80">
                        <div class="join"></div>
                        <p class="text-start">Email</p>
                        <input type="email" placeholder="username@site.com" class="input input-bordered"/>
                        <p class="text-start">Message</p>
                        <input type="text" placeholder="Type your useless questions here" class="input input-bordered"/>
                        <button type="submit" value="Send" aria-label="Submit">Submit</button>
                        <button type="reset" value="Reset" aria-label="Reset">Reset</button>
                    </fieldset>
                </form>
            </div>
        </div>
    </div>
</div>
