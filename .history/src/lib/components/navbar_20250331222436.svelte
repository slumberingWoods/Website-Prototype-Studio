<script lang="ts">
    ...
    let { data } = $props();
    let current_locale = data;
    function set_theme(locale: string) {
	    const one_year = 60 * 60 * 24 * 365;
        document.cookie = `PARAGLIDE_LOCALE=${locale}; expires=${one_year}; path=/`;
        current_locale = locale;
    }
    function toggle_locale(): void {
        const locale = current_locale === 'en' ? 'fr' : 'en';
        set_theme(locale);
        location.reload();
    }
</script>
    ...
    <div class="navbar bg-slate-900 shadow-sm content-center">
        ...
        <div class="navbar-end flex mx-12 content-center">
            ...
            <ul>            
                <div class="ml-4">
                    <form>
                        <button class="btn bg-slate-800 rounded-box" onclick={toggle_locale}>{m["button.lang"]()}</button>
                    </form>
                </div>
            </ul>
        </div>
