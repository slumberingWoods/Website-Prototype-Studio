...
<nav>
    <div class="navbar bg-slate-900 shadow-sm content-center">
        ...
        <div class="navbar-end flex mx-12 content-center">
            ...
            <ul>            
                <div class="ml-4">
                    <form method="POST" action="{localizeHref(page.url.pathname, { locale: "en"})}">
                        <button class="btn bg-slate-800 rounded-box" value="{page.url.pathname}">{m["navbar.button.lang"]()}</button>
                    </form>
                </div>
            </ul>
        </div>
